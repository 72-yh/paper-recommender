...
def _build_ivf_int8_mmap_client(tmp_path) -> TestClient:
    db_path = tmp_path / "papers.db"
    ...
    Int8VectorIndex.from_exact_index(exact).save_mmap(index_path)
    np.save(index_path / "cluster_ids.npy", np.array([0, 0], dtype=np.uint16))
    np.save(index_path / "centroids.npy", np.array([[1.0, 0.0]], dtype=np.float32))
    return TestClient(create_app(db_path=db_path, index_path=index_path, index_kind="ivf_int8_mmap"))
...
def test_recommend_endpoint_can_use_ivf_int8_mmap_index(tmp_path) -> None:
    client = _build_ivf_int8_mmap_client(tmp_path)
